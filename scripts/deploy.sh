...
if [[ "$OS" == "linux" ]]; then
    if [[ "$BUILD_WHEEL" == true ]]; then
        ...
        docker exec -i manylinux bash -c 'cd /cgra_pnr/thunder && /opt/python/cp38-cp38/bin/python setup.py bdist_wheel'
        ...
        # python 3.8 has different name now
        docker exec -i manylinux bash -c 'cd /cgra_pnr/cyclone && /opt/python/cp38-cp38/bin/python setup.py bdist_wheel'
