...
if [[ "$OS" == "linux" ]]; then
    if [[ "$BUILD_WHEEL" == true ]]; then
        ...
        docker exec -i manylinux bash -c 'cd /cgra_pnr/thunder && for PYBIN in cp38 cp39 cp310; do /opt/python/${PYBIN}-${PYBIN}/bin/python setup.py bdist_wheel; done'
        ...
        # python 3.8+ has different names now
        docker exec -i manylinux bash -c 'cd /cgra_pnr/cyclone && for PYBIN in cp38 cp39 cp310; do /opt/python/${PYBIN}-${PYBIN}/bin/python setup.py bdist_wheel; done'
