...
//  Additional Modifications @ Keyi <keyi@stanford.edu>
//
...
void NodeType() {
    ...
    for (i = 0; i < table_size; i++) {
        ...
        if (vocab[target].word[0] == 'e') {
            e_table.push_back(target);
        } else if (vocab[target].word[0] == 'i') {
            i_table.push_back(target);
        } else if (vocab[target].word[0] == 'm') {
    ...
    std::cout << "e_table.size() " << e_table.size() << std::endl;
        ...
        for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
            ...
            if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
                if (d == 0) {
                    ...
                    if (pp) {
                        if        (vocab[last_word].word[0] == 'e') {
                            target = e_table[(next_random >> 16) % e_table.size()];
                        } else if (vocab[last_word].word[0] == 'i') {
                            target = i_table[(next_random >> 16) % i_table.size()];
                        } else if (vocab[last_word].word[0] == 'm') {
...
void TrainModel() {
    ...
    if (classes == 0) {
        ...
        for (a = 0; a < vocab_size; a++) {
            if (!strncmp(vocab[a].word, "i688", 4)) {
                printf("yes");
            }
